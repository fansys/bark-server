...
func doRegister(c *fiber.Ctx, compat bool) error {
	...
	device, err := orm.SaveOrUpdateDevice(deviceInfo.DeviceToken, deviceInfo.DeviceKey)
	if err != nil {
...
func doRegisterCheck(c *fiber.Ctx) error {
	...
	deviceToken, err := orm.GetDeviceTokenByDeviceKey(deviceKey)
	if err != nil {
		return c.Status(400).JSON(failed(400, err.Error()))
	}
	if deviceToken == "" {
		return c.Status(400).JSON(failed(400, "device not registered"))
	}
	if err != nil {
		return c.Status(400).JSON(failed(400, err.Error()))
	}
